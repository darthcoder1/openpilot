...
class dbc(object):
    def __init__(self, fn):
        ...
        sg_regexp = re.compile(
            r"^SG\_ (\w+) : (\d+)\|(\d+)@(\d+)([\+|\-]) \(([0-9.+\-eE]+),([0-9.+\-eE]+)\) \[([0-9.+\-eE]+)\|([0-9.+\-eE]+)\] \"(.*)\" (.*)")
        sgm_regexp = re.compile(
            r"^SG\_ (\w+) (\w+) *: (\d+)\|(\d+)@(\d+)([\+|\-]) \(([0-9.+\-eE]+),([0-9.+\-eE]+)\) \[([0-9.+\-eE]+)\|([0-9.+\-eE]+)\] \"(.*)\" (.*)")
        val_regexp = re.compile(
            r"VAL\_ (\w+) (\w+) (\s*[-+]?[0-9]+\s+\".+?\"[^;]*)")
        ...
        for l in self.txt:
            ...
            if l.startswith("BO_ "):
                ...
                if ids in self.msgs:
                    sys.exit("Duplicate address detected %d %s" %
                             (ids, self.name))
...
if __name__ == "__main__":
    ...
    dbc_test = dbc(os.path.join(
        DBC_PATH, 'toyota_prius_2017_pt_generated.dbc'))
    msg = ('STEER_ANGLE_SENSOR', {'STEER_ANGLE': -
                                  6.0, 'STEER_RATE': 4, 'STEER_FRACTION': -0.2})
    ...
    msg = ('SAS11', {'SAS_Stat': 7.0, 'MsgCount': 0.0, 'SAS_Angle': -
                     26.200000000000003, 'SAS_Speed': 0.0, 'CheckSum': 0.0})
    encoded = dbc_test.encode(*msg)
