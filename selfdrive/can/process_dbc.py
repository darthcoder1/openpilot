...
def main():
    ...
    with open(template_fn, "r") as template_f:
        template = jinja2.Template(
            template_f.read(), trim_blocks=True, lstrip_blocks=True)
    ...
    for dbc_path in glob.iglob(os.path.join(dbc_dir, "*.dbc")):
        ...
        out_fn = os.path.join(os.path.dirname(__file__),
                              out_dir, dbc_name + ".cc")
        ...
        # remove duplicates
        def_vals = {a: set(b) for a, b in can_dbc.def_vals.items()}
        def_vals = [(address, sig)
                    for address, sig in sorted(def_vals.items())]
            ...
            for sig in sigs:
                ...
                if address in [0x200, 0x201]:
                    ...
                    if sig.name == "CHECKSUM_PEDAL" and sig.size != 8:
                        sys.exit(
                            "PEDAL CHECKSUM is not 8 bits longs %s" % msg_name)
        ...
        parser_code = template.render(
            dbc=can_dbc, checksum_type=checksum_type, msgs=msgs, def_vals=def_vals, len=len)
